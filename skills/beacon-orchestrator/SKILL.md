...
Read `../../references/PIPELINE_CONTRACT.md`, `DATA_MODEL.md`, and `THEME_MAP.md` before starting. Create one dated run directory and preserve each phase output. Keep run artifacts in `runs/YYYY-MM-DD/<slug>/`; do not place phase outputs in `state/`.

1. Call the selection, sweep, grading, historical, pattern, and Choice Brief phases in that order.
...
Never rewrite prior ledgers, replace a ledger file to add records, or convert a readiness hypothesis into a directive.
